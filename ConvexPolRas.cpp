...
void mpConvexPolRas(int *x, int *y, color* clr, int num, unsigned char alpha, mpDRAWPOINT drawPoint, void* painter)
{
	...
	for(i=0; i < delta_y; i++)
    {
        ...
        {
            //Begin inverse distance weighting interpolation for color
            ...
            for(int k = 0; k < num; k++)
            {
                dis[k]    = sqrt((j - x[k]) * (j - x[k]) + (i+small_y - y[k]) * (i+small_y - y[k]));

                if(fabs(dis[k] - 0.0) < 1e-6)
                {
                    weight[k] = 1.0;
                }
                else
            ...
            for(int k = 0; k < num; k++)
            {
                cur.R += clr[k].R * weight[k] / sum_w;
                cur.G += clr[k].G * weight[k] / sum_w;
                cur.B += clr[k].B * weight[k] / sum_w;
            }

            delete [] dis;
            delete [] weight;
            //End inverse distance weighting interpolation for color

            drawPoint(j, i+small_y, cur, painter);
        }
    }
